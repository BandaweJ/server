...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(
    ...
    this.logger.log(
      'JWT Strategy: Token decoded successfully, payload received',
    );

    const { username, role, id } = payload;

    if (!username || !role || !id) {
      this.logger.error(`JWT Strategy: Invalid payload - missing fields`, {
        username,
        role,
        id,
      });
    ...
    this.logger.debug(
      `JWT Strategy: Looking up account for username: ${username}`,
    );
    ...
    if (!user) {
      this.logger.error(
        `JWT Strategy: Account not found for username: ${username}`,
      );
      throw new UnauthorizedException('You are not Authorised');
    }

    this.logger.log(
      `JWT Strategy: Account found, looking up profile for role: ${role}, id: ${id}`,
    );
    ...
    if (role === ROLES.student && user.student) {
      ...
      if (studentNumberFromRelation && studentNumberFromRelation !== id) {
        this.logger.warn(
          `JWT Strategy: JWT id (${id}) doesn't match student number from account (${studentNumberFromRelation}). Using student number from account.`,
          {
            jwtId: id,
            accountId: user.id,
      ...
      switch (role) {
        ...
        case ROLES.dev:
          try {
            ...
            if (!profile) {
              this.logger.error('JWT Strategy: Teacher profile not found', {
                id,
                role,
              });
            ...
            if (error instanceof NotFoundException) {
              this.logger.error(
                'JWT Strategy: Teacher profile not found in database',
                {
                  id,
                  role,
                  username,
              ...
              throw new UnauthorizedException(
                `Teacher profile not found for ID: ${id}. Please contact administrator.`,
              );
            }
            // Re-throw if it's a different error
            this.logger.error(
              'JWT Strategy: Unexpected error during teacher lookup',
              {
                id,
                role,
                username,
        ...
        case ROLES.parent:
          ...
          if (!profile) {
            this.logger.error('JWT Strategy: Parent profile not found', {
              email: id,
            });
        ...
        case ROLES.student: {
          ...
          try {
            profile = await this.resourceById.getStudentByStudentNumber(
              studentNumberToUse,
            );
            if (!profile) {
            ...
            if (error instanceof NotFoundException) {
              this.logger.error(
                'JWT Strategy: Student profile not found in database',
                {
                  studentNumber: studentNumberToUse,
                  jwtId: id,
                  accountId: user.id,
              ...
              throw new UnauthorizedException(
                `Student profile not found for student number: ${studentNumberToUse}. Please contact administrator.`,
              );
            }
            // Re-throw if it's a different error
            this.logger.error(
              'JWT Strategy: Unexpected error during student lookup',
              {
                studentNumber: studentNumberToUse,
                jwtId: id,
                accountId: user.id,
